...
> **Note:** The "TV Shows" link was later changed from a plain `/` route to `/?tab=tv` once Ibrahim's Home page tabs existed (`fix: point navbar TV Shows link to home page tv tab query param`).

**Commit now with message:** `feat: add static navbar layout with nav links`

### Task 2 — Wire the search input

- [x] On submitting the search input (e.g. pressing Enter or clicking a search button), navigate to `/search?query=<the typed query>` using `useNavigate` from `react-router`.

**Acceptance criteria:** typing "batman" into the search box and submitting navigates the browser to `/search?query=batman`.

> **Note:** A guard was added to ignore empty/whitespace-only submissions (`fix(navbar): skip empty search submit`).
...
- [x] Select the wishlist array in the Navbar with `useWishlistStore((state) => state.wishlist)` from `/store/useWishlistStore.js` (a Zustand store — a small shared state container any component can read from).
- [x] Display `wishlist.length` as a live counter next to the wishlist icon in the Navbar.
- [x] Test that it updates immediately: toggle a heart icon on any card that already exists in the app (even a placeholder/foundation page), and confirm the Navbar counter changes right away without a refresh.
...
- [x] Compare the Navbar against the Figma file (`https://www.figma.com/file/jvGvsGLg6X3T7JPU3E2rNl/Movie-App`) and adjust spacing, colors, and responsive behavior (e.g. how the Navbar collapses on a narrow screen) to match — use the Tailwind design tokens Ammar set up (e.g. `bg-primary`) instead of picking your own colors.
...
> **Note:** A mobile hamburger menu (`Menu`/`X` icon toggle) was added for small screens as part of the responsive pass — not spelled out in the task text.
...
- [x] Push the branch if you haven't already (`git push -u origin feature/navbar`).
- [x] Open a Pull Request (PR — a request asking the Team Leader to review and merge your branch into `main`) from `feature/navbar` into `main`.
- [x] In the PR description, briefly state what you built (Navbar with live wishlist counter, and the dark mode toggle if you got to it) and attach a screenshot since this is a UI task. If you skipped Task 5, mention that the dark mode toggle will follow in a small separate commit once the theme store is ready.
...
- [x] Pull the latest `main` branch (it now includes your merged Navbar work).
- [x] Create your second feature branch with the exact name: `feature/search-results`.
- [x] Open `pages/SearchResults/SearchResultsPage.jsx` (Ammar already created this empty file and linked it to `/search`). Read the `query` param from the URL using `useSearchParams` from `react-router`.
- [x] Call `useSearchMovies(query, page)` from `/hooks/useSearchMovies.js` (import it exactly from that path) — it returns the standard shape `{ data, isPending, isError, error }`, with the array of results in `data.results`. Hardcode `page = 1` for now.
...
- [x] Render `data.results` as a grid of `<MovieCard item={item} mediaType="movie" />` from `/components/MovieCard.jsx` — this shared component already handles the poster, title, date, rating ring, and wishlist heart icon internally, and already navigates to `/movie/:id` via its own inner `Link` — don't wrap it in another `Link` yourself.
...
> **Note:** Unlike Mariam's Trending page (which had an explicit optional pagination task), this task never mentioned pagination as optional or required — yet real pagination was still added (`?page=` in the URL, `useSearchMovies(query, page)`, the shared `<Pagination />` component) via `feat(search): add pagination with shared Pagination component`.

**Commit now with message:** `feat: build search results page with usesearchmovies`

### Task 8 — Handle the empty/no-results state

- [x] If `data.results` comes back as an empty array, show a clear message like "No results found for '{query}'" instead of an empty grid.
...
> **Note:** An extra UX state was added for when there is no query at all yet ("Type something into the search bar to find movies.") — not just the no-results case this task asked for.
...
- [x] Compare the Search Results page against the Figma file (`https://www.figma.com/file/jvGvsGLg6X3T7JPU3E2rNl/Movie-App`) and adjust spacing, grid layout, and colors to match — use the Tailwind design tokens Ammar set up (e.g. `bg-primary`) instead of picking your own colors.
...
- [x] Push the branch if you haven't already (`git push -u origin feature/search-results`).
- [x] Open a Pull Request from `feature/search-results` into `main`.
- [x] In the PR description, briefly state what you built (Search Results page with empty state) and attach a screenshot since this is a UI task.
- [x] Tag Ammar as the reviewer.
