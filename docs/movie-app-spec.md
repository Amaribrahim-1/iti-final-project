...
- `useTrending(page)` → **bonus hook, not part of the original requirements**, built specifically to unblock Mariam's Trending page (Package D). Calls TMDB's `/trending/all/day` endpoint, `data.results` is a mixed array of movies and TV shows (each item has a `media_type` field already provided by TMDB). Takes an optional `page` argument (defaults to `1`). Unlike the AI chatbot, this hook must be built and merged as part of the foundation (Phase 0) — Mariam's whole package depends on it, so it can't be deferred the way the chatbot is.
...
- `<Layout />` — wraps every route via `<Outlet />`; holds the Navbar (filled in by Sahar).
- Two more shared components landed later during teammate packages (same folder, same rule — used by 2+ pages): `<Pagination currentPage totalPages onPageChange />` (Home, Search Results, Trending) and `<ReviewsList reviews isPending isError error />` (Movie Details and TV Show Details).
...
- **Trending page pagination**: done — `useTrending(page)` now takes a `page` argument (defaults to `1`), the Trending page reads `?page=` from the URL, and it reuses the shared `<Pagination />` component.
