...
function MovieRecommendations({ data, isPending, isError, error }) {
  const [showAllRecommendations, setShowAllRecommendations] = useState(false)
  const allRecommendedMovies = data?.results ?? []
  const recommendedMovies = showAllRecommendations
    ? allRecommendedMovies
    : allRecommendedMovies.slice(0, 6)
  ...
  return (
    <section className="mt-12">
      ...
      {isError && (
        <ErrorState
          message={error?.message || 'Failed to load recommended movies.'}
        />
      )}
      {allRecommendedMovies.length === 0 && data && (
